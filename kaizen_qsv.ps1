# --- Configuration ---
# [NO CHANGES HERE - Keep your existing paths and settings]
...
$debugWindowStyle = "Minimized" # or Normal
...
function Start-FFmpegProcess {
    ...
    if ($mode -eq "cpu") {
        ...
        $ffmpegArgs += @(
            ...
            "$mapArgs", "-map_metadata", "0",
            "-movflags", "+faststart",
            "-c:v:0", "hevc_qsv", "-preset:v", $gpuPreset, "-tag:v:0", "hvc1",
            "-profile:v:0", "main",
            "-tier:v", "high",
            # "-look_ahead:v", $gpuLookAhead, -> Only works for vbr based compresssion, not for crf/global-quality
            "-global_quality", $gpuQuality,
            # Optional/Stable QSV params (ensure these work for you)
            "-g", "250", "-bf", "7",
            # "-c:a:0", "copy"
            "-c:a:0", "aac", "-q:a:0", "3", # Audio Codec
            "-c:s", "copy"
